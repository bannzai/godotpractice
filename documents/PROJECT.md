...
1 ゲーム = `games/<slug>/` の 1 Godot プロジェクト = 1 worktree (ブランチ `game/<slug>`。品質向上ラウンドは `polish/<slug>`) = 1 作業者。仕様と受け入れ条件は各 issue を正とする。
...
上記 9 本は「最低限遊べる」ラウンド (PR #13〜#21) と「品質向上」ラウンド (PR #26〜#34。キャラ別画像・アニメーション・演出・場面ごとの音) の 2 ラウンドで作った。第 3 ラウンドは新ジャンル 10 本を、最初から品質向上ラウンドの水準で 1 ラウンドで作る (2026-09-06 開始):

| slug | 元ネタ (名称・素材は使わない) | ジャンル | 次元 | issue |
| --- | --- | --- | --- | --- |
| farmsim | Stardew Valley | 農場生活シム | 2D | #37 |
| kartrace | マリオカート | カートレース | 3D | #38 |
| towerdefense | Kingdom Rush | タワーディフェンス | 2D | #39 |
| roguelike | 不思議のダンジョン | ターン制ローグライク | 2D | #40 |
| tactics | ファイアーエムブレム | シミュレーション RPG | 2D | #41 |
| blockpuzzle | ぷよぷよ / テトリス | 落ち物パズル | 2D | #42 |
| rhythm | 太鼓の達人 | リズムゲーム | 2D | #43 |
| actionadventure | ゼルダの伝説 (2D) | 見下ろしアクションアドベンチャー | 2D | #44 |
| citybuilder | シムシティ | 都市経営シム | 2D | #45 |
| survivalcraft | Minecraft | サバイバルクラフト | 3D | #46 |
| boardrogue | bannzai 考案 (札棋 https://github.com/bannzai/boardgame-fudagi のローグライク化。IdeaMemo #222) | 盤面カード戦闘のローグライク | 2D | #60 |
| ghostrogue | bannzai 考案 (幽霊を集めて戦わせるホラー。IdeaMemo #223) | 収集と闇堕ちのローグライク | 2D | #61 |

ジャンルの選定は既存 9 本と重ならないことと、Steam の市場調査 (ローグライク・ライフシム・レーシング・ファーミング・管理シム・サバイバルクラフトの伸び) を根拠にした。boardrogue と ghostrogue は 2026-09-07 にユーザーの発案から追加した (2 つを 1 本に合わせた #58 spiritboard は取り下げ)。

品質の基準: タイトル → プレイ → 結果のループが成立し、画像・BGM・SE が入って「ゲームとして成立する」こと。素材は「それっぽければよい」で完全な再現は求めない (issue #1)。第 3 ラウンド以降はこれに加えて、品質向上ラウンドの基準 (キャラ・オブジェクトごとの画像、4 種以上のアニメーション、パーティクル・ヒットストップ・画面揺れ・遷移演出、多層背景と UI テーマ、場面ごとの BGM / SE、自動操作シナリオの録画、終了時のリーク解消、webtunnel での実操作) を各 issue の受け入れ条件に含める。
