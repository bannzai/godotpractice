...
このラウンドでは、カード定義と決闘ロジックを維持したまま、表示層を金・深緑・象牙のアール・ヌーヴォー様式へ変える。画像生成で全 30 カードの絵と共通の装飾枠を制作し、既存の手続き SVG は部位アニメーション用にだけ残す。タイトルから額縁入りのトーナメント表を経て決闘へ進み、決闘中は木製机を俯瞰する構成にする。操作案内の共通行を廃止し、選択可能・選択不可の理由と選択後の結果予告をカードや場の上へ直接表示する。初回は盤面に重ねた短いチュートリアルで、フェイズ進行・手札の使用・攻撃対象の順に案内する。
