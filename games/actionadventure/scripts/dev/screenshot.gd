...
func _capture_scenes() -> void:
	...
	main.start_new()
	await _capture("tutorial-01")
	main._next_tutorial()
	await _capture("tutorial-02")
	main._next_tutorial()
	await _capture("tutorial-03")
	main._next_tutorial()
	await _capture("field-0")
	main.world.hero.position = Vector2(340, 290)
	await _capture("interaction-available")
	main._open_map()
	await _capture("map-field")
	main._close_map()
	main.world.enter_room(6, Vector2(420, 330), false)
	main._open_map()
	await _capture("map-dungeon")
	main._close_map()
	main.world.enter_room(9, Vector2(770, 300), false)
	await _capture("interaction-unavailable-chest")
	main.world.hero.position = Vector2(1060, 384)
	await _capture("interaction-unavailable-door")
...
func _animations() -> void:
	...
	for i: int in range(names.size()):
		...
		label.add_theme_font_override("font", load("res://assets/fonts/Stick-Regular.ttf"))
