extends SceneTree
## 26秒の操作録画。終盤の出撃を初期条件とし、以降は実InputEventで射撃・回避する。
...
func _run() -> void:
	...
	if state.mode != state.Mode.RESULT or not state.cleared:
		push_error("操作録画がクリア結果まで到達していない: hp=%d" % main.boss_hp)
		failed = true
	main.stop_audio()
...
func _events() -> void:
	match tick:
		45:
			_tap(KEY_ENTER)
			# 3分全体はintegration_checkで検証し、録画では終盤の26秒へ絞る。
			state.elapsed = 140.0
			state.power = 3
