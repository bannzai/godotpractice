#!/usr/bin/env python3
"""独自の楽譜と固定 seed から、場面別の多音色 BGM / SE を再生成する。"""
...
def make_effects():
    for ordinal, name in enumerate(["card", "attack", "block", "heal", "power", "death", "transition"]):
        duration = {"card": .19, "attack": .42, "block": .58, "heal": .95,
                    "power": 1.05, "death": .85, "transition": .45}[name]
...
def main():
    ...
    make_effects()
    print("独自 BGM 6 曲、効果音 7 点を生成しました（22050 Hz / stereo PCM）。")
