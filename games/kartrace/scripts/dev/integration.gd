...
func _run() -> void:
	...
	Controls.axis(JOY_AXIS_LEFT_X, 0.8)
	await _frames(2)
	Controls.axis(JOY_AXIS_LEFT_X, 0.9)
	await _frames(2)
	_expect(_state.selected_kart == 1, "スティックを倒したままでも1車種だけ移動")
	...
	Controls.axis(JOY_AXIS_LEFT_X, 0.0)
	await _frames(2)
	_expect(_state.selected_kart == 0, "ニュートラル後は逆方向を受理")
	await _tap(KEY_F11)
	...
	_expect(_state.phase == "garage", "Enterでガレージのコース選択へ")
	await _tap(KEY_ENTER)
	_expect(_state.phase == "tutorial", "選択したポスターから初回チュートリアルへ")
	await _tap(KEY_UP)
	_expect(_state.phase == "tutorial" and _main.tutorial_step == 1,
		"アクセル入力でチュートリアルを進行")
	await _tap(KEY_RIGHT)
	_expect(_state.phase == "tutorial" and _main.tutorial_step == 2,
		"ステア入力でチュートリアルを進行")
	await _tap(KEY_SPACE)
	_expect(_state.phase == "countdown", "ドリフト入力でチュートリアルを完了")
	await _frames(190)
	...
	_main.tutorial_complete = false
	Controls.button(JOY_BUTTON_A, true)
	await _frames(2)
	Controls.button(JOY_BUTTON_A, false)
	_expect(_state.phase == "garage", "パッドAでガレージへ進む")
	Controls.button(JOY_BUTTON_A, true)
	await _frames(2)
	Controls.button(JOY_BUTTON_A, false)
	_expect(_state.phase == "tutorial", "パッドAでコースを選択")
	Controls.button(JOY_BUTTON_A, true)
	await _frames(2)
	Controls.button(JOY_BUTTON_A, false)
	_expect(_state.phase == "countdown", "パッドAでチュートリアルをスキップ")
	await _frames(190)
