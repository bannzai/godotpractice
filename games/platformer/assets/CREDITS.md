...
- **クレジット表記**: **不要**
- **改変**: キャラクターシートの明るい無彩色またはマゼンタ背景を実行時シェーダで透明化し、濃紺の輪郭を追加。背景画像は画面比率に合わせて Godot でトリミング表示
...
- **生成**: OpenAI built-in `image_gen` / プロンプトの要点: 太い濃紺の輪郭、セル塗り、ハーフトーン、スクリーン印刷の質感を共通仕様にし、青緑・黄土・珊瑚・クリーム・紫の配色で、空の配達人、風の塊、結晶甲虫、浮島の手描き地図、風車草原、発光洞窟を制作。歩行敵は6列×5行・各セル内へ収めた異なる30ポーズを再生成。固有作品の再現を求めず、文字・ロゴ・透かしを禁止した。
