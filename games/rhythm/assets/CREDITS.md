...
- **クレジット表記**: **必要**
- **改変**: 既存ゲームと同一のフォントを無改変で複製。著作権表示と OFL 全文を同梱。
- **備考**: 入手元: https://github.com/google/fonts/tree/main/ofl/mplusrounded1c 。フォントと OFL.txt は公開リポジトリへの再配布条件を満たすため同梱。画面上のクレジット表示は任意。
