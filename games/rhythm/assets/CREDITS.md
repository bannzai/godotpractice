...
- **素材名**: 祭り囃子の楽曲・境内環境音・効果音 (`starlight.ogg`, `moonride.ogg`, `comet.ogg`, `title.ogg`, `select.ogg`, `festival-ambience.ogg`, `result-clear.ogg`, `result-fail.ogg`, `hit-coral.wav`, `hit-mint.wav`, `hold.wav`, `fever.wav`, `miss.wav`)
- **作者**: 本プロジェクトで新規作成（Codex によるコード生成）
- **入手 URL**: https://github.com/bannzai/godotpractice
- **ライセンス**: 外部素材未使用の独自生成物。CC0 等の別ライセンスは付与していない。
- **クレジット表記**: **不要**
- **改変**: scripts/dev/generate_assets.py から新規生成。外部の画像・楽曲・音声サンプルは不使用。
- **備考**: 再生成: python3 scripts/dev/generate_assets.py。OpenAI と利用者との関係では適用法の範囲で出力の権利は利用者に帰属: https://openai.com/policies/terms-of-use/ 。既存作品・ブランドの再現なし。
- **生成**: Python 3 標準ライブラリ / ffmpeg / Codex / プロンプトの要点: 112 BPM / 72 秒、128 BPM / 75 秒、144 BPM / 70 秒の 3 曲。和太鼓の面・縁、笛、三味線風の決定的な合成音色を曲と操作音へ配置。境内環境音は滑らかなざわめき、拍子木、遠花火、風鈴を外部サンプルなしで合成。音源の時刻 0 が第 0 拍。
